...
- `Pipeline` sets the export type, organised into groups by usage and file type
	...
	- **Volume (3D Texture)**
		...
		- `PNG — 3D Texture Strip` exports a 3D texture strip PNG file with normalised 0-1 range
			...
			- This format isn't ideal, but in special cases may work for compressed delivery and re-expansion to -/+ values in-engine (and sometimes it's just easier to review with remapped ranges)
